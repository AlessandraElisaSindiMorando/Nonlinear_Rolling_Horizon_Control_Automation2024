function [x_seq, mv] = NonLinear_MPC_with_Obstacles_CasaDi_moreObstacles(x0, xs, u0, ...
    detected, Xobs)
%NONLINEAR_MPC_WITH_OBSTACLES_CASADI This function implement a 
% Nonlinear Model Predictive Control (MPC). In particular
...
%   Xobs: (x,y)-position of the detected obstacles, it should be passed
...
% family of constraints is added to the NMPC formulation
 n_obstacles = size(Xobs, 2);
if detected > 0
    for i = 1:n_obstacles    
        for k = 1:(Hp+1)
            g = [g; (X(1:2,k) - Xobs(:,i))'*(X(1:2,k) - Xobs(:,i))];
        end
    end
end
...
if detected > 0
    args.lbg(n_states*(Hp+1)+1+1:...
        (n_obstacles+n_states)*(Hp+1)+1)  = (d_Safe+epsilon)^2;
    args.ubg(n_states*(Hp+1)+1+1:...
        (n_obstacles+n_states)*(Hp+1)+1) = inf;
end
